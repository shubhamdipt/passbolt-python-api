...
class PassboltAPI:
    def __init__(self, config_path, new_keys=False, delete_old_keys=False):
        ...
        try:
            self.gpg_fingerprint = [
                i for i in self.gpg.list_keys() if i["fingerprint"] ==
                self.config["PASSBOLT"]["USER_FINGERPRINT"]
            ][0]["fingerprint"]
        except IndexError:
            raise Exception(
                "GPG public key could not be found. Check: gpg --list-keys")

        if self.config["PASSBOLT"]["USER_FINGERPRINT"] not in [
    ...
    def _import_gpg_keys(self):
        if not self.config["PASSBOLT"]["USER_PUBLIC_KEY_FILE"]:
            raise ValueError(
                "Missing value for USER_PUBLIC_KEY_FILE in config.ini")
        if not self.config["PASSBOLT"]["USER_PRIVATE_KEY_FILE"]:
            raise ValueError(
                "Missing value for USER_PRIVATE_KEY_FILE in config.ini")
        self.gpg.import_keys(
            open(self.config["PASSBOLT"]["USER_PUBLIC_KEY_FILE"], "r").read())
        self.gpg.import_keys(
            open(self.config["PASSBOLT"]["USER_PRIVATE_KEY_FILE"], "r").read())
    ...
    def get_headers(self):
        return {
            ...
            if "csrfToken" in self.requests_session.cookies else ""
        }
    ...
    def post(self, url, data):
        r = self.requests_session.post(self.server_url + url,
                                       json=data,
                                       headers=self.get_headers())
        return r.json()

    def put(self, url, data):
        r = self.requests_session.put(self.server_url + url,
                                      json=data,
                                      headers=self.get_headers())
        return r.json()
